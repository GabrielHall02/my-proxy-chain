...
import { isUserAllowed, subtractBandwidth } from './utils/apiHelpers';
...
export class Server extends EventEmitter {
    ...
    registerConnection(socket: Socket): void {
        ...
        socket.on('close', () => {
            const clientIp = socket.remoteAddress as string;
            const stats = this.getConnectionStats(unique) as ConnectionStats;
            ...
            // Subtract bandwidth from the user
            const ip = clientIp.split(':').pop();
            if (ip) {
                try {
                    subtractBandwidth(ip, stats['srcRxBytes'] / 1000 + stats['srcTxBytes'] / 1000);
                } catch (error) {
                    console.error(`Error subtracting bandwidth from ${ip}:`, error);
                }
    ...
    onConnection(socket: Socket): void {
        ...
        try {
            if (!isUserAllowed(socket.remoteAddress)) {
                console.log('User not allowed', socket.remoteAddress);
                socket.destroy();
                return;
            }
        } catch (error) {
    ...
    async onRequest(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
        try {
            ...
            if (request.socket && request.socket.remoteAddress) {
                console.log("REQUESTING", request.socket.remoteAddress)
                try {
                    if (!await isUserAllowed(request.socket.remoteAddress)) {
                        console.log('User not allowed', request.socket.remoteAddress);
                        response.destroy();
                        return;
                    }
                } catch (error) {
    ...
    async onConnect(request: http.IncomingMessage, socket: Socket, head: Buffer): Promise<void> {
        try {
            if (socket && socket.remoteAddress) {
                ...
                try {
                    if (!await isUserAllowed(socket.remoteAddress)) {
                        console.log('User not allowed', socket.remoteAddress);
                        socket.destroy();
                        return;
                    }
